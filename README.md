# Claude Monetization Growth Prototypes

Hi-fi clickable prototypes for three Claude monetization concepts, embedded in a faithful Claude UI shell.

**Live walkthrough:** https://ambrishverma.github.io/claude-growth-prototypes/

## The three prototypes

1. **Dynamic Offers** — A cohort engine surfaces the right offer at the right moment, in-product, in admin, and over email.
2. **Power Weeks** — A 7-day Pro trial that earns its conversion: invitation → day-4 nudge → end-of-trial paywall.
3. **Sprint Packs** — Limit-hit moments become opt-in, time-boxed sprints of unlimited Claude, with bundling into Max tiers.

## Stack

Pure static HTML/CSS + React 18 transpiled in-browser via Babel standalone. No build step. Any static host works.

## Local preview

```sh
python3 -m http.server 8765
# → open http://localhost:8765
```
